# -*- coding: utf-8 -*-
"""
Invoice Extract - FastAPI Backend
...
async def parse_document(file: UploadFile = File(...)):
    """Parse a document using TextIn official API. Returns markdown + pages."""
...
def _parse_json_from_text(text: str) -> Any:
    """Extract JSON object from LLM response text (handles markdown code blocks)."""
    text = text.strip()
...
async def extract(req: ExtractRequest):
    """Call OpenAI-compatible LLM with document markdown and extraction prompt."""
    ...
    # Return same format as original internal API for frontend compatibility
